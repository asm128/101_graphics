...
int							drawBuffer				(::HDC hdc, const SColor* target, SCoord sizeTarget)				{
	...
	for(uint32_t y = 0, yMax = sizeTarget.y; y < yMax; ++y)	// This inverts the Y axis of the source by copying to temporary memory which is later sent to the temporary bitmap trough SetDIBits().
		memcpy(&offscreenDetail.BitmapInfo->bmiColors[y * sizeTarget.x], &target[(sizeTarget.y - 1 - y) * sizeTarget.x], sizeTarget.x * sizeof(SColor));
	...
	if(offscreenDetail.IntermediateDeviceContext) {
		...
		if(0 != offscreenDetail.IntermediateBitmap) {	// We  need to avoid this
			::SetDIBits(NULL, offscreenDetail.IntermediateBitmap, 0, sizeTarget.y, offscreenDetail.BitmapInfo->bmiColors, offscreenDetail.BitmapInfo, DIB_RGB_COLORS);
